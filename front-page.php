...
<section class="relative overflow-hidden pt-10 pb-20 px-6">
    ...
    <!-- Upcoming Events -->
<section class="py-10 bg-background-light dark:bg-background-dark celtic-pattern">
        ...
        <div class="flex overflow-x-auto gap-6 pb-8 snap-x no-scrollbar">
            ...
            if ($events->have_posts()) :
                while ($events->have_posts()) : $events->the_post();
                    ...
                    $border_color = $type === 'match' ? 'border-vienna-gold' : 'border-primary';
                    $badge_color = $type === 'match' ? 'bg-vienna-gold/10 text-vienna-gold' : 'bg-primary/10 text-primary';
            ?>
            <a href="<?php the_permalink(); ?>" class="min-w-[300px] bg-white dark:bg-neutral-800 rounded-2xl p-6 soft-lift snap-center border-l-4 <?php echo $border_color; ?> hover:-translate-y-2 transition-all duration-300 block group">
                <div class="flex justify-between items-start mb-4">
                    <span class="<?php echo $badge_color; ?> px-3 py-1 rounded-full text-[10px] font-bold uppercase"><?php echo esc_html($type); ?></span>
